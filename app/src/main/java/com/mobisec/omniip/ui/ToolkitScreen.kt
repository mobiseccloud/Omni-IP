...
@Composable
fun DnsThreatFeedsScreen(viewModel: com.mobisec.omniip.viewmodel.AppMatrixViewModel = androidx.lifecycle.viewmodel.compose.viewModel(), onRequirePremium: () -> Unit = {}) {
        ...
        Button(
            onClick = {
                if (!viewModel.isEnterpriseUnlocked.value && !viewModel.isPersonalUnlocked.value) {
                    onRequirePremium()
                } else if (customDomain.isNotBlank()) {
